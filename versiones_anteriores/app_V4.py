...
def delete_files(delete_df: pd.DataFrame, protected_roots: List[str], dry_run: bool = True) -> Tuple[List[str], List[str]]:
    success_messages: List[str] = []
    error_messages: List[str] = []
...
def render_delete_section(df: pd.DataFrame, dry_run: bool, protected_roots: List[str]) -> None:
    ...
    if not delete_plan.empty and delete_target:
        st.warning(
            ...
            f"La app solo intentará eliminar subcarpetas internas vacías y protegerá las carpetas raíz ingresadas. "
        ...
        if st.button("Confirmar acción", type="primary", use_container_width=True, disabled=not confirm):
            success_messages, error_messages = delete_files(delete_plan, protected_roots=protected_roots, dry_run=dry_run)
