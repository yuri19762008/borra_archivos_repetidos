...
def safe_file_size(file_path: str) -> Optional[int]:
    try:
        return os.path.getsize(file_path)
    except (PermissionError, FileNotFoundError, OSError) as exc:
        LOGGER.error("Error obteniendo tamaño de %s: %s", file_path, exc)
        return None
...
def collect_files(folder_path: str, progress_bar=None, status_text=None) -> Tuple[List[Dict], List[str]]:
    ...
    for root, _, files in os.walk(folder_path):
        ...
        if progress_bar is not None:
            progress_value = min(processed_dirs / total_dirs, 1.0)
            progress_bar.progress(progress_value, text=f"Escaneando: {root}")
...
def summarize_duplicates(df: pd.DataFrame) -> Tuple[int, int]:
    ...
    recoverable = (
        df.groupby("group_id")
        .apply(lambda group: group["size_bytes"].iloc[0])
        .sum()
    )
...
def render_delete_section(df: pd.DataFrame, dry_run: bool) -> None:
    ...
    if not delete_plan.empty and delete_target:
        ...
        if st.button("Confirmar acción", type="primary", use_container_width=True, disabled=not confirm):
            ...
            for msg in success_messages[:20]:
                st.success(msg)
            if len(success_messages) > 20:
                st.info(f"Se generaron {len(success_messages)} mensajes de éxito. Se muestran solo los primeros 20.")
...
def main() -> None:
    ...
    if scan_info:
        ...
        st.write(
            f"Carpeta 1: **{scan_info['folder_1']}** | Archivos encontrados: **{scan_info['files_folder_1']}**"
        )
        st.write(
            f"Carpeta 2: **{scan_info['folder_2']}** | Archivos encontrados: **{scan_info['files_folder_2']}**"
        )
        st.write(
